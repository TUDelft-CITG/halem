...
def test_costfunction_time():
    ...
    WVPI= 1
    L = Functions.costfunction_timeseries(edge, vship(), WD_min, flow(3), WVPI, 1,  flow(3).tria)
...
def test_costfunction_space():
    ...
    mask = np.full((u(mag).shape), False)
    WVPI= 1
    L = Functions.costfunction_spaceseries(edge, vship(), WD_min, flow(3), WVPI, 1,  flow(3).tria)
    dist1 = Functions.haversine(coord_a(), coord_b())
    dist = dist1
