...
# Halem simulaiton classes
import halem.simulation as halem_core
...
def env(FG, Roadmap):
    ...
    simulation_start = datetime.datetime.fromtimestamp(d.timestamp())
...
def test_halem_single_path(env, locatable_a, locatable_b):
    ...
    path_halem, _, _ = halem.HALEM_time(
        (locatable_a.geometry.x, locatable_a.geometry.y),
        (locatable_b.geometry.x, locatable_b.geometry.y),
        env.t0,
        vmax,
        env.Roadmap,
    )
