...
class Movable(SimpyObject, Locatable):
    ...
    def get_distance(self, origin, destination):
        ...
        if hasattr(self.env, 'FG') and isinstance(self, Routeable):
        ...
        elif hasattr(self.env, 'FG') and isinstance(self, Routeable) and hasattr(self.env, 'Roadmap'):
            dista = 0
            # Origin is geom - convert to node on graph
            geom = nx.get_node_attributes(self.env.FG, 'geometry')
            for node in geom:
            ...
            for node in enumerate(route):
                ...
                dista += self.wgs84.inv(orig.x, orig.y, dest.x, dest.y)[2]
                sailtime += RODFF.get_sailtime((orig.x, orig.y), (dest.x, dest.y), (self.env.now + sailtime), self.current_speed, self.env.Roadmap)
                
                self.log_entry("Sailing", self.env.now + sailtime, self.current_speed, dest)
            ...
            self.log_entry("Sailing", self.env.now, self.current_speed, orig)
